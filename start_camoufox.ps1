...
function Wait-PythonGracefulShutdown($Process, [int]$GraceSeconds) {
    ...
    Write-Host ""
    Write-Info "收到 Ctrl+C，已通知 Python 停止；等待 Python 停止新任务并关闭当前 Camoufox 浏览器..."
    ...
    Write-Info "Python 未在 ${GraceSeconds}s 内退出，强制终止 Python 子进程及其 Camoufox 浏览器进程..."
...
function Invoke-Python([string]$PythonExe, [string[]]$Arguments) {
    ...
    $stopFilePath = Join-Path ([System.IO.Path]::GetTempPath()) ("qwen-register-camoufox-stop-{0}.signal" -f ([guid]::NewGuid().ToString("N")))
    try {
...
function Invoke-InterruptCleanupSelfTest {
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("qwen-start-camoufox-interrupt-selftest-{0}" -f ([guid]::NewGuid().ToString("N")))
...
try {
    ...
    $logFile = Join-Path $logDir "qwenv4-camoufox.log"
