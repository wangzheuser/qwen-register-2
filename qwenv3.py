...
def get_current_ip(context):
    """Get the current public IP and country being used (via proxy if active)"""
    import json
    ...
    try:
        ...
        page2.goto(f'https://ipinfo.io/{ip}/json', wait_until='domcontentloaded', timeout=20000)
        body2 = page2.evaluate('document.body.innerText') or ''
        page2.close()
...
def main():
    ...
    with sync_playwright() as p:
        for i in range(num_accounts):
            ...
            # Detect public IP being used
            current_ip, country = get_current_ip(context)
            print(f"  📡 Current IP: {current_ip} ({country})")

            # Generate credentials
            first_name = gen_first_name()
            prefix = gen_prefix(first_name)
            ...
            try:
                ...
                if verify_url:
                    ...
                    try:
                        ...
                        time.sleep(5)
                        qwen.screenshot(path=f'{IMAGES_DIR}/qwen_verified_{i+1}.png')

                        body = qwen.evaluate('document.body.innerText') or ''
                        print(f"  📋 Verify result: {body[:200]}")

                        save_account(email, password, name, current_ip, country)
                        success_count += 1
